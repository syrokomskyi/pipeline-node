...
const createHarness = async () => {
  ...
  const artifacts = new Map([
    ["build", { result: { kind: "file" as const, relativePath: "result.txt" } }],
  ]);
  const ctx = createNodePipelineContext({
    workspaceRootDir: root,
    inputDir: path.join(root, "input"),
    outputDir: path.join(root, "output"),
    promptsDir: path.join(root, "prompts"),
    stepArtifactsById: artifacts,
    stepNumbers: new Map([["build", 1]]),
    state: {},
    services: {},
  });
...
const contract = (
  operation: string,
  completion?: PipelineFingerprintContract["completion"],
): PipelineFingerprintContract => ({
  schema: "pipeline-fingerprint-contract@1",
  executionSemantics:
    completion?.kind === "human_decision"
      ? "human_gate"
      : completion?.kind === "external_receipt"
        ? "external_effect"
        : "pure_artifact",
...
test("legacy and tampered outputs are never reusable", async () => {
  ...
  await expect(
    ctx.isStepReusable?.({ stepId: "build", artifacts: ["result"], fingerprint: contract("v1") }),
  ).resolves.toBe(false);
...
test("operation changes invalidate only the declared consumer manifest", async () => {
  ...
  await expect(
    ctx.isStepReusable?.({ stepId: "build", artifacts: ["result"], fingerprint: contract("v2") }),
  ).resolves.toBe(false);
});

test("declared filesystem inputs cannot escape the workspace boundary", async () => {
  const { ctx } = await createHarness();
  const fingerprint: PipelineFingerprintContract = {
    ...contract("bounded"),
    implementationInputs: async () => [
      { kind: "file", id: "escaped", path: path.join(os.tmpdir(), "outside-workspace.ts") },
    ],
  };

  await expect(ctx.resolveStepFingerprint?.({ stepId: "build", fingerprint })).rejects.toThrow(
...
test("human completion must bind the decision to the reviewed fingerprint", async () => {
  ...
  await ctx.writeTextFile(
    ctx.getStepArtifactPath("build", "result"),
  ...
  await expect(
    ctx.isStepReusable?.({
      stepId: "build",
      artifacts: ["result"],
      fingerprint: contract("changed", { kind: "human_decision", artifactId: "result" }),
    }),
  ).resolves.toBe(false);
});
...
test("external completion rejects a local receipt without an idempotency key", async () => {
  ...
  await expect(
    ctx.recordStepCompletion?.({ stepId: "build", artifacts: ["result"], fingerprint }),
  ).rejects.toThrow("has no matching idempotency key");
});
...
test("external completion accepts a receipt bound to the operation fingerprint", async () => {
  ...
  await ctx.writeTextFile(
    ...
    JSON.stringify({
      ...
      idempotencyKey: resolved?.operationFingerprint,
      externalId: "remote-42",
    }),
  );
  await ctx.recordStepCompletion?.({ stepId: "build", artifacts: ["result"], fingerprint });
  await expect(
    ctx.isStepReusable?.({ stepId: "build", artifacts: ["result"], fingerprint }),
  ).resolves.toBe(true);
});
...
test("a step output transaction atomically promotes validated staging bytes", async () => {
  ...
  await expect(fs.readFile(ctx.getStepArtifactPath("build", "result"), "utf8")).resolves.toBe(
    "new\n",
  );
});
